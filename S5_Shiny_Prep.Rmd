...
# Files
HELCA <- readRDS(file = "helca_31Oct.rds")
...
HELCA <- HELCA %>% 
  rename("Demographic.Disabled" = "Disability",
         ...
         "Demographic.Income.Decile" = "Income.Decile",
         "Yeargroups" = "Year")
...
# test the sample, should be 8000
HELCA_24 <- HELCA %>% 
...
# Rename Insurance variable so clear
HELCA <- HELCA %>%
  select(-Expenditure.Vehicle.Insurance) %>% 
  rename("Expenditure.Vehicle.Insurance" = "Expenditure.Vehicle.Insurance.weekly")
...
# Get rid of ones not suited for the app
HELCA <- HELCA %>% 
  select(-c(A020,A021,A022,A030,A031,A032))
...
# Save final file
saveRDS(HELCA, file = "appdf31_10_2.rds")
