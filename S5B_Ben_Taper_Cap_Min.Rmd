...
# Let's pop this in provisionally
tom_finished <- tom_finished %>%
  mutate(
...
tom_finished <- tom_finished %>%
  mutate(
    Demographic.Fishy = case_when(
      is.na(Exp_Inc_Difference) ~ 'Unknown',  # Handling NA values in Exp_Inc_Difference
      Exp_Inc_Difference >= 4 ~ 'Timmy',
      Income.Total == 0 ~ 'Fishy',
      Expenditure.Total == 0 ~ 'Fishy',
      Expenditure.Utilities == 0 ~ 'Fishy',
      Expenditure.Food.House.Keeping == 0 ~ 'Fishy', # take out source
      TRUE ~ 'Not so fishy'
    )
  )
...
tom_finished <- tom_finished %>%
  mutate(
    Demographic.FishyV2 = case_when(
      is.na(Exp_Inc_Difference) ~ 'Unknown',  # Handling NA values in Exp_Inc_Difference
      Exp_Inc_Difference >= 4 & Demographic.Means.Tested != "MTB" ~ 'Timmy',
      Income.Total == 0 ~ 'Fishy',
      Expenditure.Total == 0 ~ 'Fishy',
      Expenditure.Utilities == 0 ~ 'Fishy',
      Expenditure.Food.House.Keeping == 0 ~ 'Fishy', # take out source
...
tom_finished <- tom_finished %>%
  mutate(
    Demographic.FishyV3 = case_when(
      ...
      Expenditure.Total == 0 ~ 'Fishy',
      Expenditure.Utilities == 0 ~ 'Fishy',
      Expenditure.Food.House.Keeping == 0 ~ 'Fishy',
      TRUE ~ 'Not so fishy'
    )
  )

test <- tom_finished %>% 
  filter(Demographic.Fishy == "Timmy",
         Demographic.Cohort == "2023-24P")
hist(test$Expenditure.Total)
table(tom_finished$Demographic.Fishy, tom_finished$Demographic.Cohort)

# test <- tom_finished %>% 
#   filter(is.na(Income.Total)) %>% 
#   select(Income.Total, Income.Total.Benefits, Income.Gross.Wages.Partner, Income.Gross.Wages.Respondent,
#          Income.Takehome.Wages.Respondent, Income.Takehome.Wages.Partner)
...
# Demographic.Number.of.Child.LCFS"               "Demographic.Number.of.Dependent.Child.LCFS"    
#  [19] "Demographic.Number.of.Non.Dependent.Child.LCFS" "Demographic.Number.of.Child.<1.LCFS"            "Demographic.Number.of.Child.<5.LCFS"           
#  [22] "Demographic.Number.of.Child.5-9.LCFS"           "Demographic.Number.of.Child.10-15.LCFS"         "Demographic.Number.of.Child.<16.LCFS"          
#  [25] "Demographic.Number.of.Child.16-18.LCFS"  
